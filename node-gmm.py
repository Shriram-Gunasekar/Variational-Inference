...
def guide(x, y):
    ...
    with pyro.plate('data', len(x)):
        assignment = pyro.sample('assignment', dist.Categorical(torch.ones(num_components) / num_components))
        means = pyro.sample('means', dist.Normal(0, 10).expand([num_components]).to_event(0))
        scales = pyro.sample('scales', dist.LogNormal(0, 1).expand([num_components]).to_event(0))
        component_dist = dist.Normal(means[assignment], scales[assignment])
        pyro.sample('obs', component_dist, obs=y)
